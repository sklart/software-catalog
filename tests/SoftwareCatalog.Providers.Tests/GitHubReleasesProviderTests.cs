...
public sealed class GitHubReleasesProviderTests
{
    ...
    [Fact] public async Task ResolveReturnsSingleInstallerAndFiltersNonInstallers() { const string json = """{"tag_name":"v2.0","assets":[{"name":"Tool-2.0-x64.exe","browser_download_url":"https://example.test/Tool-2.0-x64.exe","size":12,"content_type":"application/octet-stream"},{"name":"Tool-portable.zip","browser_download_url":"https://example.test/portable.zip"},{"name":"Tool-symbols.zip","browser_download_url":"https://example.test/symbols.zip"},{"name":"checksums.txt","browser_download_url":"https://example.test/checksums.txt"},{"name":"Source code.zip","browser_download_url":"https://example.test/source.zip"}]}"""; var result = await Provider(HttpStatusCode.OK, json).ResolveAsync(Product(), Source(), null, CancellationToken.None); var candidate = Assert.Single(result.Candidates); Assert.Equal(DownloadCandidateStatus.Available, result.Status); Assert.Equal("Tool-2.0-x64.exe", candidate.FileName); Assert.Equal("x64", candidate.Architecture); }
    [Fact] public async Task ResolveMapsAmbiguousNotFoundAndHttpErrors() { const string ambiguous = """{"tag_name":"v2","assets":[{"name":"ToolSetup.exe","browser_download_url":"https://example.test/a.exe"},{"name":"ToolInstaller.exe","browser_download_url":"https://example.test/b.exe"}]}"""; Assert.Equal(DownloadCandidateStatus.Ambiguous, (await Provider(HttpStatusCode.OK, ambiguous).ResolveAsync(Product(), Source(), null, CancellationToken.None)).Status); Assert.Equal(DownloadCandidateStatus.NotFound, (await Provider(HttpStatusCode.OK, "{\"tag_name\":\"v2\",\"assets\":[]}").ResolveAsync(Product(), Source(), null, CancellationToken.None)).Status); Assert.Equal(DownloadCandidateStatus.Error, (await Provider(HttpStatusCode.InternalServerError, "{}").ResolveAsync(Product(), Source(), null, CancellationToken.None)).Status); }
