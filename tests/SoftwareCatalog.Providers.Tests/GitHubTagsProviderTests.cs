...
public sealed class GitHubTagsProviderTests
{
    ...
    [Fact] public async Task TimeoutIsClassified(){var provider=new GitHubTagsProvider(new HttpClient(new ThrowingHandler(new TaskCanceledException())){BaseAddress=new Uri("https://api.github.com/")},new ProductNormalizer());var result=await provider.CheckLatestAsync(Product(),Source(),CancellationToken.None);Assert.Equal(UpdateStatus.Error,result.Status);Assert.Equal(ProviderErrorKind.Timeout,result.ErrorKind);}
