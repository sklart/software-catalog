...
public sealed class CatalogScannerTests
{
    ...
    [Fact] public async Task WriterFailureReturnsErrorAndNextScanWorks()
    {
        var folder = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("N")); Directory.CreateDirectory(folder); await File.WriteAllTextAsync(Path.Combine(folder, "setup.exe"), "one");
        try { var repo = new MemoryRepository { FailNextWrite = true }; var root = await repo.AddScanRootAsync(folder, ScanRootPathKind.Absolute, true, CancellationToken.None); var scanner = new CatalogScanner(repo, new Resolver(), new FakeHash(), new CatalogScannerOptions { MaxDegreeOfParallelism = 1 }); var first = await scanner.ScanAsync(root, null, CancellationToken.None).WaitAsync(TimeSpan.FromSeconds(5)); Assert.False(first.Completed); Assert.Contains(first.Errors, error => error.Message.Contains("writer", StringComparison.OrdinalIgnoreCase)); var second = await scanner.ScanAsync(root, null, CancellationToken.None).WaitAsync(TimeSpan.FromSeconds(5)); Assert.True(second.Completed); } finally { Directory.Delete(folder, true); }
    }
    [Fact] public async Task HashFailureReturnsErrorAndNextScanWorks()
    {
        var folder = Path.Combine(Path.GetTempPath(), Guid.NewGuid().ToString("N")); Directory.CreateDirectory(folder); await File.WriteAllTextAsync(Path.Combine(folder, "setup.exe"), "one");
        try { var repo = new MemoryRepository(); var root = await repo.AddScanRootAsync(folder, ScanRootPathKind.Absolute, true, CancellationToken.None); var hash = new FailingHash(); var scanner = new CatalogScanner(repo, new Resolver(), hash, new CatalogScannerOptions { MaxDegreeOfParallelism = 1 }); var first = await scanner.ScanAsync(root, null, CancellationToken.None).WaitAsync(TimeSpan.FromSeconds(5)); Assert.False(first.Completed); Assert.Contains(first.Errors, error => error.Message.Contains("hash failure", StringComparison.OrdinalIgnoreCase)); hash.Fail = false; var second = await scanner.ScanAsync(root, null, CancellationToken.None).WaitAsync(TimeSpan.FromSeconds(5)); Assert.True(second.Completed); } finally { Directory.Delete(folder, true); }
    }
