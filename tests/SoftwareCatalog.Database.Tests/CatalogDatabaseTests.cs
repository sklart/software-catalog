...
public sealed class CatalogDatabaseTests : IAsyncLifetime
{
    ...
    [Fact] public async Task MigrationAndConstraintsAreEnforced()
    {
        ...
        await using (var migration = connection.CreateCommand()) { migration.CommandText = "SELECT MAX(version) FROM schema_migrations"; Assert.Equal(4L, Convert.ToInt64(await migration.ExecuteScalarAsync())); }
    ...
    [Fact] public async Task InitializerUpgradesRealVersionTwoDatabaseToVersionThreeAndIsIdempotent()
    {
        ...
        await database.InitializeAsync(CancellationToken.None); await using var check = new SqliteConnection($"Data Source={path};Pooling=False"); await check.OpenAsync(); await using var verify = check.CreateCommand(); verify.CommandText = "SELECT group_concat(version, ',') FROM schema_migrations ORDER BY version"; Assert.Equal("1,2,3,4", await verify.ExecuteScalarAsync()); Assert.Single(await database.GetInstallersAsync(CancellationToken.None));
    }
    [Fact] public async Task InitializerUpgradesVersionThreeDatabaseToVersionFourWithoutChangingMetadata()
    {
        ...
        await upgraded.InitializeAsync(CancellationToken.None); await using var check = new SqliteConnection($"Data Source={path};Pooling=False"); await check.OpenAsync(); await using var verify = check.CreateCommand(); verify.CommandText = "SELECT group_concat(version, ',') FROM schema_migrations ORDER BY version"; Assert.Equal("1,2,3,4", await verify.ExecuteScalarAsync()); Assert.Single(await upgraded.GetInstallersAsync(CancellationToken.None));
    }
