...
public sealed class CatalogDatabaseTests : IAsyncLifetime
{
    ...
    [Fact] public async Task FailingMigrationRollsBackAndPreservesOriginalError()
    {
        ...
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => new MigrationRunner([new FailingMigration()], null).ApplyAsync(connection, CancellationToken.None)); Assert.Equal("intentional migration failure", exception.Message);
        await using var check = connection.CreateCommand(); check.CommandText = "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='rollback_probe';"; Assert.Equal(0L, Convert.ToInt64(await check.ExecuteScalarAsync())); check.CommandText = "SELECT COUNT(*) FROM schema_migrations WHERE version=99;"; Assert.Equal(0L, Convert.ToInt64(await check.ExecuteScalarAsync()));
    }
