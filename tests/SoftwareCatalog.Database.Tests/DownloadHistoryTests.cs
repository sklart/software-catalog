...
public sealed class DownloadHistoryTests : IAsyncLifetime
{
    ...
    public async Task RealV5ToV6PreservesCompleteStageThreeAndUpdateState()
    {
        ...
        await using var verify = new SqliteConnection($"Data Source={Path};Pooling=False"); await verify.OpenAsync(); await using var versions = verify.CreateCommand(); versions.CommandText = "SELECT group_concat(version, ',') FROM (SELECT version FROM schema_migrations ORDER BY version)"; Assert.Equal("1,2,3,4,5,6,7,8", await versions.ExecuteScalarAsync()); versions.CommandText = "SELECT COUNT(*) FROM download_history"; Assert.Equal(0L, Convert.ToInt64(await versions.ExecuteScalarAsync()));
    }
}
