...
public sealed class Migration006FullUpgradeTests : IAsyncLifetime
{
    ...
    public async Task ManuallyCreatedFullV5DatabaseUpgradesToV6WithoutChangingAnyStageThreeData()
    {
        ...
        await using var connection = new SqliteConnection($"Data Source={DatabasePath};Pooling=False"); await connection.OpenAsync(); await using var command = connection.CreateCommand(); command.CommandText = "SELECT group_concat(version, ',') FROM (SELECT version FROM schema_migrations ORDER BY version)"; Assert.Equal("1,2,3,4,5,6,7,8,9", await command.ExecuteScalarAsync()); command.CommandText = "SELECT COUNT(*) FROM download_history"; Assert.Equal(0L, Convert.ToInt64(await command.ExecuteScalarAsync()));
    }
