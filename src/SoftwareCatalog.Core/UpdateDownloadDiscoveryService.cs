...
public sealed class UpdateDownloadDiscoveryService(IProductCatalogRepository repository, IEnumerable<IUpdateDownloadProvider> providers, IAppLogger? logger = null)
{
    public async Task<DownloadCandidateResolution> ResolveAsync(SoftwareProduct product, UpdateCheckResult? update, CancellationToken token)
    {
        ...
        try
        {
            ...
            if (result.Status is DownloadCandidateStatus.Error or DownloadCandidateStatus.NotFound or DownloadCandidateStatus.Unsupported) return result;
            ...
            if (architectures.Length != 1) return result.Candidates.Count == 1 ? result with { Status = DownloadCandidateStatus.Available } : result with { Status = DownloadCandidateStatus.Ambiguous, Error = "Недостаточно сведений об архитектуре локального продукта." };
            ...
            return chosen.Length switch { 0 => new(DownloadCandidateStatus.Unsupported, [], "Нет совместимого installer для архитектуры продукта."), 1 => new(DownloadCandidateStatus.Available, chosen), _ => new(DownloadCandidateStatus.Ambiguous, chosen, "Несколько равноправных installer-файлов.") };
        }
