...
public sealed class UpdateBatchService(IUpdateChecker discovery, IAppLogger? logger = null)
{
    public async Task<UpdateBatchResult> CheckAsync(IEnumerable<SoftwareProduct> products, bool force, int cacheHours, int maxParallelism, CancellationToken cancellationToken)
    {
        ...
        await Task.WhenAll(list.Select(async product => { await gate.WaitAsync(cancellationToken); try { cancellationToken.ThrowIfCancellationRequested(); var result = await discovery.CheckAsync(product, force, cacheHours, cancellationToken); lock (results) results.Add(result); } finally { gate.Release(); } }));
