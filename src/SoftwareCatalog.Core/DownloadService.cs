...
public sealed class DownloadService(HttpClient client, IAppLogger? logger = null)
{
    public async Task<DownloadResult> DownloadAsync(DownloadCandidate candidate, string stagingDirectory, string destinationDirectory, IProgress<DownloadProgress>? progress, TimeSpan timeout, CancellationToken token)
    {
        ...
        try
        {
            ...
            if (response.RequestMessage?.RequestUri is { Scheme: var finalScheme } && !finalScheme.Equals(Uri.UriSchemeHttps, StringComparison.OrdinalIgnoreCase)) return new(DownloadStatus.Error, null, null, "Redirect на не-HTTPS URL запрещён.");
            ...
            { var buffer = new byte[131072]; int count; while ((count = await input.ReadAsync(buffer, linked.Token)) > 0) { await output.WriteAsync(buffer.AsMemory(0, count), linked.Token); received += count; var seconds = Math.Max(.001, (DateTimeOffset.UtcNow - started).TotalSeconds); progress?.Report(new(received, total, total is > 0 ? received * 100d / total : null, received / seconds, DownloadStatus.Downloading, name)); } await output.FlushAsync(linked.Token); }
            if (total is not null && total != received) return new(DownloadStatus.Error, null, null, "Размер скачанного файла не совпадает с Content-Length.");
    ...
    public static string SanitizeFileName(string? value) { if (string.IsNullOrWhiteSpace(value) || value.Contains("..", StringComparison.Ordinal) || Path.IsPathRooted(value)) return "download.bin"; var name = Path.GetFileName(value).Trim().TrimEnd('.', ' '); foreach (var c in Path.GetInvalidFileNameChars()) name = name.Replace(c, '_'); if (string.IsNullOrWhiteSpace(name) || IsReserved(name)) return "download.bin"; return name; }
