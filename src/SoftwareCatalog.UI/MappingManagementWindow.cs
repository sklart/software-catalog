...
public sealed class MappingManagementWindow : Window
{
    ...
    public MappingManagementWindow(IReadOnlyList<ProductUpdateSource> mappings, DateTimeOffset? lastCheckedUtc, Func<ProductUpdateSource,Task> save, Func<ProductUpdateSource,Task> remove, Func<Task> searchCandidates)
    {
        ...
        var actions=new MappingManagementActions(save,remove,searchCandidates); var externalId=new TextBox { Width=280 }; _grid.SelectionChanged+=(_,_)=>{if(_grid.SelectedItem is ProductUpdateSource item)externalId.Text=item.ExternalId;};
        var enable=new Button{Content="Enable"}; enable.Click+=async(_,_)=>{if(_grid.SelectedItem is ProductUpdateSource item){var value=item with{Enabled=true};await actions.EnableAsync(item);Replace(item,value);}}; var disable=new Button{Content="Disable",Margin=new Thickness(8,0,0,0)};disable.Click+=async(_,_)=>{if(_grid.SelectedItem is ProductUpdateSource item){var value=item with{Enabled=false};await actions.DisableAsync(item);Replace(item,value);}};var change=new Button{Content="Change",Margin=new Thickness(8,0,0,0)};change.Click+=async(_,_)=>{if(_grid.SelectedItem is ProductUpdateSource item && !string.IsNullOrWhiteSpace(externalId.Text)){var value=item with { ExternalId=externalId.Text.Trim() };await actions.ChangeExternalIdAsync(item,externalId.Text);Replace(item,value);}};var delete=new Button{Content="Remove",Margin=new Thickness(8,0,0,0)};delete.Click+=async(_,_)=>{if(_grid.SelectedItem is ProductUpdateSource item){await actions.RemoveAsync(item);_items.Remove(item);}};var search=new Button{Content="Search candidates",Margin=new Thickness(8,0,0,0)};search.Click+=async(_,_)=>await actions.SearchCandidatesAsync();var close=new Button{Content="Закрыть",IsCancel=true,Margin=new Thickness(8,0,0,0)};var buttons=new StackPanel{Orientation=Orientation.Horizontal,HorizontalAlignment=HorizontalAlignment.Right,Margin=new Thickness(0,8,0,0)};buttons.Children.Add(new TextBlock{Text="External ID:",VerticalAlignment=VerticalAlignment.Center});buttons.Children.Add(externalId);foreach(var button in new[]{enable,disable,change,delete,search,close})buttons.Children.Add(button);var panel=new DockPanel{Margin=new Thickness(12)};DockPanel.SetDock(buttons,Dock.Bottom);panel.Children.Add(buttons);panel.Children.Add(_grid);Content=panel;
    }
