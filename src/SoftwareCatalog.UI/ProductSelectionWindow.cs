...
public sealed class ProductSelectionWindow : Window
{
    ...
    public ProductSelectionWindow(IEnumerable<SoftwareProduct> products, SoftwareProduct? current)
    {
        ...
        _grid = new DataGrid { ItemsSource = products.OrderBy(product => product.CanonicalName).ToArray(), SelectedItem = current, AutoGenerateColumns = false, IsReadOnly = true, MinHeight = 360 };
