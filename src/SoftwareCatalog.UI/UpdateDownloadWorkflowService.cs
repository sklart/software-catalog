...
public sealed class UpdateDownloadWorkflowService(DownloadService downloader, DownloadCoordinator coordinator, IDownloadHistoryRepository history, IProductCatalogRepository products, IScanCatalogRepository scans, CatalogScanner scanner, ProductCatalogService catalog, IAppPathService appPaths, AppSettings settings, ProductMatchingService matching)
{
    ...
    private async Task<DownloadWorkflowResult> ExecuteCoreAsync(SoftwareProduct product, DownloadCandidate candidate, IProgress<DownloadProgress>? progress, CancellationToken token)
    {
        ...
        try
        {
            ...
            {
                if (await history.HasInstallerSha256Async(result.Sha256, token)) { File.Delete(result.FinalPath); result = new(DownloadStatus.AlreadyExists, null, result.Sha256, "Такой SHA-256 уже есть в каталоге."); }
        ...
        catch (OperationCanceledException) { result = new(DownloadStatus.Cancelled, null, null, "Отменено."); }
        catch (Exception ex) { result = new(DownloadStatus.Error, null, null, ex.Message); }
    ...
    private async Task<DownloadStatus> ValidateAsync(SoftwareProduct product, string stagedPath, CancellationToken token)
    {
        ...
        if (metadata.Status != MetadataStatus.Success || string.IsNullOrWhiteSpace(metadata.ProductName) || string.IsNullOrWhiteSpace(metadata.ProductVersion)) return DownloadStatus.ManualConfirmationRequired;
        ...
        if (match?.ProductId != product.Id && !nameMatch) return DownloadStatus.ValidationFailed;
        if (!string.IsNullOrWhiteSpace(product.LatestNormalizedVersion) && new VersionComparer().Compare(probe.NormalizedVersion, product.LatestNormalizedVersion) == VersionComparisonResult.Older) return DownloadStatus.ValidationFailed;
    ...
    private async Task<DownloadResult> ImportAsync(DownloadCandidate candidate, DownloadResult staged, string destination, CancellationToken token)
    {
        ...
        await scanner.ScanAsync(root, null, token); await catalog.RegroupAsync(token); return finalized;
    }
}
