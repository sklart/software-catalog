...
public sealed class GitHubTagsProvider(HttpClient client, ProductNormalizer normalizer) : IUpdateProvider
{
    ...
    public async Task<UpdateCheckResult> CheckLatestAsync(SoftwareProduct product, ProductUpdateSource? source, CancellationToken token)
    {
        ...
        try { using var response=await client.GetAsync($"repos/{source.ExternalId}/tags?per_page=1",token); if(response.StatusCode==HttpStatusCode.NotFound)return new(UpdateStatus.NotFound,Source:Id,ExternalProductId:source.ExternalId,ErrorKind:ProviderErrorKind.NotFound); if(!response.IsSuccessStatusCode)return new(UpdateStatus.Error,Source:Id,ExternalProductId:source.ExternalId,Error:$"GitHub returned {(int)response.StatusCode}",ErrorKind:ProviderErrorKind.NetworkError); var tags=await response.Content.ReadFromJsonAsync<Tag[]>(cancellationToken:token); var tag=tags?.FirstOrDefault()?.name; return string.IsNullOrWhiteSpace(tag) ? new(UpdateStatus.NotFound,Source:Id,ExternalProductId:source.ExternalId,ErrorKind:ProviderErrorKind.NotFound) : new(UpdateStatus.Unknown,tag,normalizer.NormalizeVersion(tag),tag,null,new Uri($"https://github.com/{source.ExternalId}/tags"),Id,source.ExternalId); } catch(OperationCanceledException) when(!token.IsCancellationRequested){return new(UpdateStatus.Error,Source:Id,ExternalProductId:source.ExternalId,Error:"GitHub request timed out",ErrorKind:ProviderErrorKind.Timeout);} catch(HttpRequestException ex){return new(UpdateStatus.Error,Source:Id,ExternalProductId:source.ExternalId,Error:ex.Message,ErrorKind:ProviderErrorKind.NetworkError);} catch(System.Text.Json.JsonException){return new(UpdateStatus.Error,Source:Id,ExternalProductId:source.ExternalId,Error:"Malformed GitHub tags response",ErrorKind:ProviderErrorKind.InvalidResponse);}
    }
