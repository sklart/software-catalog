...
dotnet build "$root\SoftwareCatalog.slnx" -c Release --no-restore -m:1
dotnet test "$root\SoftwareCatalog.slnx" -c Release --no-build -m:1
dotnet restore "$root\src\SoftwareCatalog.UI\SoftwareCatalog.UI.csproj" -r win-x64 -m:1
dotnet publish "$root\src\SoftwareCatalog.UI\SoftwareCatalog.UI.csproj" -c Release -r win-x64 --self-contained true --no-restore -m:1 -o $output
